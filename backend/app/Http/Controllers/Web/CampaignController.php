...
class CampaignController extends Controller
{
    ...
    public function store(Request $r)
    {
        $r->validate([
            ...
            'unit' => 'required|string|in:kg,pcs,liter,butir',
            'buyer_unit_price' => 'required|integer|min:1',
        ...
        Campaign::create([
            ...
            'buyer_unit_price' => $r->buyer_unit_price,
            'unit' => $r->unit,
            'target_quantity' => $r->target_quantity,
            'deadline' => now()->addDays($r->deadline_days),
    ...
    public function show(string $uuid)
    {
        $campaign = Campaign::with([
            'initiator:id,name',
            'cluster:id,name',
            'variants:id,campaign_id,name,package_quantity,max_quantity,sold_quantity'
    ...
    {
        $campaign = Campaign::with('variants:id,campaign_id,name,package_quantity,max_quantity,sold_quantity')
            ->where('uuid', $uuid)
    ...
    public function update(Request $r, string $uuid)
    {
        ...
        $campaign->update($r->only(['title', 'target_quantity', 'buyer_unit_price', 'location_distribution']));
    ...
    public function cancel(string $uuid)
    {
        ...
        \Illuminate\Support\Facades\Cache::forget("dashboard_metrics_initiator_".auth()->id());
    ...
    {
        $campaign = Campaign::with(['variants', 'orders.user:id,name'])
            ->where('uuid', $uuid)
            ->firstOrFail();
