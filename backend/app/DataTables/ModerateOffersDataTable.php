...
class ModerateOffersDataTable extends DataTable
{
        ...
        return (new EloquentDataTable($query))
            ...
            ->addColumn('action', function($o) {
                return '<div class="flex gap-2">'
                    . '<form method="POST" action="' . route('admin.offers.moderate-action', $o->id) . '" class="inline"><input type="hidden" name="_token" value="' . csrf_token() . '"><input type="hidden" name="action" value="approve"><button type="submit" class="btn btn-sm btn-primary"><i data-lucide="check" class="w-4 h-4 mr-1"></i>Approve</button></form>'
                    . '<form method="POST" action="' . route('admin.offers.moderate-action', $o->id) . '" class="inline"><input type="hidden" name="_token" value="' . csrf_token() . '"><input type="hidden" name="action" value="reject"><button type="submit" class="btn btn-sm btn-danger"><i data-lucide="x" class="w-4 h-4 mr-1"></i>Reject</button></form>'
                    . '</div>';
            })
            ->rawColumns(['action'])
            ->setRowId('id');
    ...
    public function query(SupplierOffer $model): QueryBuilder { return $model->newQuery()->with(['product','supplier'])->where('status', 'pending_moderation')->latest(); }
