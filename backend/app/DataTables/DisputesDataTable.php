...
class DisputesDataTable extends DataTable
{
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->editColumn('status', fn($po) => '<span class="badge badge-' . ($po->status === 'rejected' ? 'danger' : 'warning') . '">' . ucfirst($po->status) . '</span>')
            ->editColumn('created_at', fn($po) => $po->created_at->diffForHumans())
            ->addColumn('action', fn($po) => '<a href="' . route('admin.disputes.show', $po->id) . '" class="btn btn-sm btn-primary"><i data-lucide="eye" class="w-4 h-4 mr-1"></i>Detail</a>')
    ...
    {
        return $model->newQuery()->with(['campaign','initiator','supplier'])->whereIn('status', ['disputed','rejected'])->latest();
    }
    ...
    public function html(): HtmlBuilder
    {
        return $this->builder()->setTableId('disputes-table')->columns($this->getColumns())
            ...
            ->parameters(['language'=>['search'=>'Cari dispute:','lengthMenu'=>'Tampilkan _MENU_','info'=>'_START_-_END_ dari _TOTAL_ dispute'], 'responsive'=>true]);
    }
